...
class TestEndToEndLiveCosmos(IsolatedAsyncioTestCase):
    ...
    async def test_payload_uploaded(self, mock_oracle_conn, mock_oracle):
        ...
        uploader = LiveUploader(oracle, CosmosTable.LEVEL_1_SOILMET_30MIN, sites, _BUCKET, bucket_prefix=_BUCKET_PREFIX, app_prefix=_APP_PREFIX)
    ...
    async def test_multiple_payloads_uploaded_for_site(self, mock_oracle_conn, mock_oracle):
        ...
        payload = [{"DATE_TIME": datetime(2025, 1, 2), "data1": 2.4},
...
class TestCLI(TestCase):
    ...
    @patch("iotswarm.db.Oracle.query_datetime_gt_from_site", AsyncMock(return_value=[{"DATE_TIME": datetime(2025, 1, 2), "data1": 2.4},
        {"DATE_TIME": datetime(2025,1,3), "data1": 11}]))
    def test_all(self):
        ...
        with runner.isolated_filesystem():
            result = runner.invoke(cli, ["send-live-data", str(_CONFIG_SRC), "--table", "all", "--site", sites[0], "--site", sites[1]])
